//
// Script para cargar la gráfica correspondiente
// deacuerdo a cada selección hecha por el usuario
// --------------------------------------------------
...
(function() {
	...
	var switchTarget$ = $('#marcoVisualizaciones');
	var $title = $('#graphTitle');
	var $type = $('#graphType');
	var $url = $('#graphBDD');

	// Obtiene valores ingresados y obtiene la gráfica correspondiente
	$('#createGraph').on('click', function (e) {
		e.preventDefault();
		...
		if (title === "" || url === "") {
			alert("Por favor escriba un título y una url donde se encuetran los datos para graficar");
			return false;
		} else {
			switch($type.val()) {
				case("bar"):
					switchTarget$.attr('src', "barras/auto.html?title=" + title + "?bdd=" + url);
					break;
				case("com"):
					//
					break;
	...
	// Obtiene el url completo del iframe, para ser copiado y puesto en cualquier web
	$("#btnEmbeber").click(function () {
		$("#compartir").empty();
		$("#compartir").fadeToggle("slow", function () {
			var fuente = $('#marcoVisualizaciones').contents().get(0).location.href;
			$("#compartir").text('<iframe src="' + fuente + '" frameborder="0" scrolling="no" style="overflow: hidden; width: 100%; height: 640px;"></iframe>');
		});
	});
})();